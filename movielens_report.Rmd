...
We can use a machine learning algorithm to try and determine the optimal value of k, and then to create the smaller $P_{n \times k}$ and $Q_{m \times k}$ matrices, based on the known values of $R_{m \times n}$. As previously, the training set will be used to train the algorithm, the accuracy of which will then be tested on the test set.  
...
$$\displaystyle \min_{P,Q} \sum_{m,n}{{\Large[}(r_{m,n}-\mathbf{p_m}\mathbf{q_n}^T)^2 + \lambda_p ||\mathbf{p_m}||^2 + \lambda_q ||\mathbf{q_n}||^2{\Large]}}$$
where $||\mathbf{.}||$ is the euclidian norm, $(m,n) \in \mathbb{R}^2$ indicates that rating $r_{m,n}$ is available, $\lambda_p$ and $\lambda_q$ are regularization coefficients to avoid over-fitting errors.  
...
#Create the results vector and compare to actuals
rmse_results <- bind_rows(tibble(method = "Movie + user effect model + genre effect, penalised, separate lambdas, constrained", RMSE = mugceffect_rmse),
...
* One based on a linear regression, where we ended with an RMSE of `r round(mugceffect_rmse, 3)`
